...
const ProductImageGallery = ({ images, productId }) => {
  ...
  const addtoCartHandler = async () => {
    if (!isLoggedIn && !check) {
      ...
      const alreadyInCart = items.some(item => item.id === productId);
      if (!alreadyInCart) {
      ...
      <div className="flex md:flex-col gap-3 w-full  space-y-2 mr-4 md:w-24">
        {images?.map((img, index) => (
          <img
            ...
            className={`w-16 h-20 border cursor-pointer object-contain ${img === selectedImage ? 'border-blue-500' : 'border-gray-300'
              }`}
          />
        ))}
      </div>
